...
begin
  ...
  repo_url      = %(#{config['repo_site']}/#{user}/#{config['repo_name']})
...
if File.exist?(File.join(PROJECT_ROOT, pe_pkg))
  ...
  on 'master', %(wget --progress=bar --content-disposition https://s3.amazonaws.com/pe-builds/released/#{version}/#{pe_pkg})
